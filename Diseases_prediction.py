...
with st.sidebar:           
    selected = option_menu('Health Guard Prediction System',
                           ...
                           icons=['house','activity', 'heart-half', 'capsule','shield-shaded','person-fill'],
                           default_index=0)
#About home page
if selected == 'Home':
    st.title('||  Health Guard Prediction System  ||')
...
# Insert custom CSS to change the color 
    ...
    def get(path:str):
        with open(path,'r') as p:
            return json.load(p)
    path=get('C:/Users/Ritik/3D Objects/Health Guard System/Saved_models/animation.json')
    st_lottie(path,speed=1,reverse=False,loop=True,quality='medium',height=350,width=700,key=None)

    

    st.write("The Health Guard Prediction System represents a pioneering fusion of machine learning technology and healthcare expertise, poised to redefine disease prevention and management. At its core, this system acts as a digital sentinel, continuously monitoring an individual's health profile, including medical history, genetic predispositions, lifestyle choices, and environmental factors. Through sophisticated algorithms and predictive modeling techniques, Health Guard sifts through this wealth of data, uncovering subtle patterns and correlations that may elude human observation.")
    st.write("By leveraging advanced machine learning algorithms such as logistic regression, K-Nearest Neighbour, or Support Vector Machine(SVM), Health Guard can generate highly accurate predictions regarding an individual's susceptibility to various diseases. These predictions encompass a wide spectrum of health conditions, ranging from common chronic illnesses like diabetes and cardiovascular disease to more rare and genetically influenced disorders.")
    st.write("However, Health Guard goes beyond mere prediction; it provides actionable insights to empower individuals in taking control of their health destinies. Armed with this knowledge, individuals can proactively mitigate their risk factors, adopt healthier habits, and engage in early intervention strategies, potentially averting the onset or progression of serious health conditions.")
    st.write("In essence, the Health Guard Prediction System epitomizes the transformative potential of Machine Learning in healthcare. By harnessing the power of data-driven insights, personalized predictions, and proactive interventions, it heralds a new era of preventive medicine, where individuals are empowered to take charge of their health and well-being like never before.")
    st.markdown('* #### Problem:')
    st.markdown("The current diagnostic process for many diseases is slow, expensive, and often inaccurate. This leads to delayed treatment and poor patient outcomes. HealthGuard aims to address these issues by providing a  faster, more accurate diagnosis.")
    st.markdown('* #### Solution:')
    st.markdown("Health Guard uses machine learning algorithms to analyze patient data and detect 5 diseases with high accuracy. By leveraging the power of ML algorithm, Health Guard provides a faster and  more accurate diagnosis, leading to better patient outcomes.")
    st.markdown("* #### Tech. Information:")
    st.markdown("Health Guard uses advanced machine learning algorithms to analyze patient data, including medical history, symptoms, and lab results.")
    st.markdown("* #### Result Accuracy:")
    st.markdown("Health Guard has been extensively tested and has shown high levels of  accuracy in detecting 5 diseases, including Diabetes, Heart Disease, Breast Cancer, Liver Disease, and Parkinson's Disease. This accuracy is due  to the advanced algorithms patterns that Health Guard uses.")
